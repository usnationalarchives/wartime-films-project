        ...
        protected override void OnElementChanged(ElementChangedEventArgs<WebViewCustom> e)
        {
            ...
            if (e.NewElement != null)
            {
                if (Element.Source != null)
                {
                    ...
                    Control.LoadRequest(new NSUrlRequest(new NSUrl(source.BaseUrl)));
                }
            }
        }
        ...
        public override void DecidePolicy(WKWebView webView, WKNavigationAction navigationAction, Action<WKNavigationActionPolicy> decisionHandler)
        {
            try
            {
                ...
                if (request.Url.AbsoluteString != "about:blank" && canGoForward && !request.Url.AbsoluteString.Contains("vimeo") && !request.Url.AbsoluteString.StartsWith("file:///"))
                {
                    ...
                    if (!request.Url.AbsoluteString.Contains(NaraTools.Host) && !request.Url.AbsoluteString.Contains("nara://") && !request.Url.AbsoluteString.Contains("login") && !request.Url.AbsoluteString.Contains("vimeo"))
                    {
        ...
        public override void DidFinishNavigation(WKWebView webView, WKNavigation navigation)
        {
            ...
            if (webView.IsLoading == false)
            {
                ...
                if (custom_WebView != null)
                {
                    ...
                    //Task.Run(() =>
                    //{
                    //    repo.SaveContent(new OfflineContent() { Content = content, Date = DateTime.Now, Url = reqUrl });
            ...
            try
            {
                if (webUtil.CheckLogin())
                {
                    ...
                    if (cookiesR != null)
                    {
                        ...
                        {
                            cookiesS += string.Format("document.cookie = '{0}={1}; expires={2}; domain={3};';", cookie.Name, cookie.Value, cookie.Expires.ToUniversalTime()
        .ToString("ddd, dd MMM yyyy HH:mm:ss 'GMT'"), cookie.Domain);
